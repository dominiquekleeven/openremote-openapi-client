...
class AttributeWriteResult(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["ref", "failure"]

    @field_validator('failure')
    def failure_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ASSET_NOT_FOUND', 'ATTRIBUTE_NOT_FOUND', 'INSUFFICIENT_ACCESS', 'INVALID_VALUE', 'INTERCEPTOR_FAILURE', 'STATE_STORAGE_FAILED', 'CANNOT_PROCESS', 'QUEUE_FULL', 'UNKNOWN', 'ASSET_ID_MISSING', 'ATTRIBUTE_NAME_MISSING']):
            raise ValueError("must be one of enum values ('ASSET_NOT_FOUND', 'ATTRIBUTE_NOT_FOUND', 'INSUFFICIENT_ACCESS', 'INVALID_VALUE', 'INTERCEPTOR_FAILURE', 'STATE_STORAGE_FAILED', 'CANNOT_PROCESS', 'QUEUE_FULL', 'UNKNOWN', 'ASSET_ID_MISSING', 'ATTRIBUTE_NAME_MISSING')")
        return value
