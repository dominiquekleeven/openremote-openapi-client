...
class ProvisioningConfigObjectObject(BaseModel):
    ...
    def user_roles_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['read:admin', 'read:logs', 'read:users', 'read:map', 'read:assets', 'read:rules', 'read:insights', 'read:alarms', 'write:user', 'write:admin', 'write:logs', 'write:assets', 'write:attributes', 'write:rules', 'write:insights', 'write:alarms', 'read', 'write']):
                raise ValueError("each list item must be one of ('read:admin', 'read:logs', 'read:users', 'read:map', 'read:assets', 'read:rules', 'read:insights', 'read:alarms', 'write:user', 'write:admin', 'write:logs', 'write:assets', 'write:attributes', 'write:rules', 'write:insights', 'write:alarms', 'read', 'write')")
        return value
    ...
    def from_dict(cls, obj: Dict[str, Any]) -> Optional[Union[X509ProvisioningConfig]]:
        ...
        if object_type ==  'X509ProvisioningConfig':
            return import_module("openremote_openapi_client.models.x509_provisioning_config").X509ProvisioningConfig.from_dict(obj)
