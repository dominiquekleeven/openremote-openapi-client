# coding: utf-8

"""
    OpenRemote Manager REST API

    This is the documentation for the OpenRemote Manager HTTP REST API.  Please see the [documentation](https://docs.openremote.io) for more info.

    The version of the OpenAPI document: 3.0.0
    Contact: info@openremote.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501



import unittest

from or_rest_client.models.node import Node

class TestNode(unittest.TestCase):
    """Node unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> Node:
        """Test Node
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `Node`
        """
        model = Node()
        if include_optional:
            return Node(
                id = '',
                type = 'INPUT',
                priority = 56,
                name = '',
                position = or_rest_client.models.node_position.NodePosition(
                    x = 1.337, 
                    y = 1.337, ),
                size = or_rest_client.models.node_position.NodePosition(
                    x = 1.337, 
                    y = 1.337, ),
                internals = [
                    or_rest_client.models.node_internal.NodeInternal(
                        name = '', 
                        picker = or_rest_client.models.picker.Picker(
                            type = 'TEXT', 
                            options = [
                                or_rest_client.models.option.Option(
                                    name = '', 
                                    value = or_rest_client.models.value.value(), )
                                ], ), 
                        value = or_rest_client.models.value.value(), 
                        break_type = 'NEW_LINE', )
                    ],
                inputs = [
                    or_rest_client.models.node_socket.NodeSocket(
                        id = '', 
                        name = '', 
                        type = 'NUMBER', 
                        node_id = '', 
                        index = 56, )
                    ],
                outputs = [
                    or_rest_client.models.node_socket.NodeSocket(
                        id = '', 
                        name = '', 
                        type = 'NUMBER', 
                        node_id = '', 
                        index = 56, )
                    ],
                display_character = ''
            )
        else:
            return Node(
        )
        """

    def testNode(self):
        """Test Node"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
